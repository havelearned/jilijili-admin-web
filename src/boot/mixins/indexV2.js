...
import {downFilePlus} from "@/boot/api/reqGateway";
...
export const IndexMixin = {
    ...
    methods: {
        ...
        query(props) {
            ...
            this.loading = true;
            return this.$axiosPlus.get(this.url.list, {
                params: {
                    ...this.queryParam(),
                    ...this.searchForm,
                    keyword: this.key,
        ...
        deleteBatch() {
            this.loading = true;
            return this.$axiosPlus.delete(this.url.deleteBatch, {
        ...
        submit() {
            ...
            if (this.editType === '新建') {
                q = this.$axiosPlus.post(this.url.add, this.form);
            } else {
                console.log("更新表单=>", this.url.edit, this.form)
                q = this.$axiosPlus.put(this.url.edit, this.form);
            }
            return q.then((r) => {
        ...
        copy(p) {
            this.$q.dialog({
                ...
                this.$axiosPlus.post(`${this.url.copy}?id=${p.id}&name=${data}`, {}).then((r) => {
                    this.$info(r.message);
                    this.query();
                });
            });
