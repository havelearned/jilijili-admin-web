      ...
      >
        ...
        <template v-slot:item="props">
          <div class="q-ma-xs" @mouseover="showBtnGroup(props.row.index)"
            ...
            <q-card style="width: 292px">
              <q-separator/>
              <q-img :src="props.row.albumCover" height="200px" ratio="2"
                     no-native-menu>
              ...
              <q-btn-group push class="q-pa-lg-md" :id="`btnGroup-`+props.row.index" v-show="isMouseOver">
                <q-btn color="grey-7" size="ms">
                  ...
                  <q-checkbox v-model="selected" :val="props.row.albumId"/>
                </q-btn>
                <q-btn color="yellow-10" icon="updated" @click="myEdit(props.row)">
                  <q-tooltip>修改</q-tooltip>
                </q-btn>
                <q-btn color="negative" icon="deleted" @click="del({id:props.row.albumId})">
        ...
        <template #body-cell-opt="props">
          <q-td :props="props" :auto-width="true">
            ...
            <btn-del label="歌手信息" @confirm="deleted(props.row)"/>
          </q-td>
        </template>
      </q-table>
    </div>
                ...
                <div>
                  ...
                  <div>
                    ...
                    <q-avatar class="q-pa-md" size="150px">
                      <img :src="form.albumCover">
                    </q-avatar>
                  </div>

                  <h5>选择歌手:</h5>
                  <selected v-model="form.singerIds"
                            :add-url="url.singerAddUrl" :list-url="url.singerListUrl"></selected>
                  <h5>专辑名称:</h5>
                  <q-input outlined dense square clearable v-model="form.albumName"
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {name: 'albumName', align: 'center', label: '专辑名称', field: 'albumName',},
        {name: 'singerIds', align: 'center', label: '歌手', field: 'singerIds',},
        {name: 'desc', align: 'center', label: '简介', field: 'desc',},
        {name: 'albumCover', align: 'center', label: '专辑封面', field: 'albumCover',},
      ...
      url: {
        ...
        delete: '/album',
        deleteBatch: '/album',
        exportXlsUrl: '/album/export',
        importExcelUrl: '/album/import',
        getImportTemplate: '/album/get/exportTemplate',
        singerAddUrl: '/singer',
  ...
  methods: {
    ...
    initDict() {
      // 获取歌手名称和名称
      this.$axios.get(this.url.singerListUrl).then(res => {
        res.data.records.forEach(item => {
          this.singerOption.push({
            label: item.name,
            value: item.singerId,
          })
        })
    ...
    uploadPhoto() {
      ...
      this.$axios.post(this.uploadUrl, formdata, {
        ...
        console.log(res)
        if (res.code === 200) {
          this.form.albumCover = res.data[0]
        }
      }).catch(error => {
    ...
    filterFn(val, update) {
      ...
      update(() => {
        this.$axios.get(this.url.singerListUrl, {params: param}).then(res => {
          res.data.records.forEach(item => {
            this.singerOption.push({
              label: item.name,
              value: item.singerId,
      ...
      console.log("options=>", this.singerOption)
    },
    // 提交前
    mySubmit() {
      if (!this.form.singerIds) {
        this.submit()
        return
      }
      this.form.singerIds = this.form.singerIds.map(singer => singer.value)
      this.submit()
    },
    // 修改强
    myEdit(row) {
