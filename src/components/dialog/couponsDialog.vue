<template>
  <div>
    <q-dialog ref="couponsDialog" flat persistent>
      ...
      methods: {
        ...
        yes() {
          ...
          this.$emit('selected', this.selected.map(item=>{
            return {label:item.otherCouponInfo,value:item.couponId}
          }))
          this.selected=[];
        },
        show() {
          this.$refs.couponsDialog.show();

        },
        hide() {
          this.$refs.couponsDialog.hide();
        }
